...
def get_boundary_points(model,
    ...
    if 'pgd' in pipeline:
        ...
        if backend == 'tf.keras':
            ...
            elif isinstance(search_range[2], list):
                boundary_points = []
                success = 0.
                for i in trange(0, x.shape[0], batch_size):
            ...
            print(f"boundary points: f{boundary_points.shape}")
...
class BA_pytorch(object):
    def __init__(self, attribution, use_boundary=True):
        ...
        if attribution in ['BIG', 'big', 'bIG', 'Big', 'Boundary-based_IG']:
            ...
            raise NotImplementedError(f"{attribution} is not implemented yet. Please make a Issue on our github: https://github.com/zifanw/boundary")
    ...
    def boundary_search(model, x, y_onehot, pipeline, **kwargs):
        ...
        if 'CW' in methods:
            ...
            for i in trange(0, cw_bd_x.shape[0], batch_size):
                ...
                _, pred = torch.max(model(batch_x.to(device)).detach().cpu().data, 1)
